...
def when_done(this, out, comparative=False, my=False):
    ...
    if my:
        output_mystats(this, out, massaged_dups, r_payoffs, s_payoffs, comparative=comparative, splitat=splitat)
    else:
...
def my_when_done(this, out):
    """ Actually calculates the statistics

    """

    when_done(this, out, my=True)


def my_when_done_comparative(this, out):
    """ Actually calculates the statistics

    """

    when_done(this, out, my=True, comparative=True)
...
def misused(msg, information_content):
    return any(any(info_content2 < 0.
                    for state2, info_content2 in enumerate(information_content)
                    if state2 != state) and info_content > 0.
                for state, info_content in enumerate(information_content))
...
def my_measures(this, sender_pop, receiver_pop, n=2):
    msgs = list(itertools.product(range(n), range(n)))
    states = list(itertools.product(range(n), range(n)))
    state_probs = [1. / float(len(states))] * len(states)
    ...
    all_cprobs_msg_given_state = collections.defaultdict(list)
    all_cprobs_state_given_msg = collections.defaultdict(list)
    information_contents = collections.defaultdict(my_dict_factory) 
    for i, msg in enumerate(msgs):
        cprobs_msg_given_state = []
        for j, state in enumerate(states):
            pr = 0.
        ...
        for j, state in enumerate(states):
            ...
            for (sender, sender_prob) in sender_pop:
                if sender_pop > this._effective_zero: 
                    if prob_state_given_msg > 0. and not math.isinf(prob_state_given_msg):
                        s_matrix = simulation.sender_matrix(sender)
                        sender_prob_state_given_msg = ((state_probs[j] * float(s_matrix[j][i])) /
                                                        math.fsum(state_probs[k] * float(s_matrix[k][i]) 
                                                                    for k in xrange(len(states))))
...
def output_mystats(this, out, duplications, r_payoffs, s_payoffs, comparative=False, splitat=None):
    ...
    for dup_i, (final_sender, final_receiver, generations) in enumerate(duplications):
        ...
        if not options.quiet:
            ...
            for msg, cprobs_state_given_msg in all_cprobs_state_given_msg.iteritems():
                print >> out, "\t{0:>5}: {1}".format(msg, cprobs_state_given_msg)
            print >> out, "My Information Measures (sender: msg: J(msg): [J(msg, state) for state in states]):"
        ...
        messages_misused = []
        for sender, information_contents in kls.iteritems():
            messages_misused = messages_misused + [(sender, msg) for msg, information_content in information_contents.iteritems()
                                                                    if misused(msg, information_content)]

        if len(messages_misused) > 0:
            times_misused += 1
        ...
        for (sender, msg) in messages_misused:
            sender_prob = [sprob for (sender2, sprob) in final_sender if sender2 == sender][0]
            s_matrix = simulation.sender_matrix(sender)
            states_msg_sent = [state for state, row in enumerate(s_matrix) if row[msg] == 1]
            for (receiver, receiver_prob) in final_receiver:
                if comparative and receiver < splitat:
    ...
    print >> out, "Number of duplications with misuse: {0}".format(times_misused)
    print >> out, "Number of duplications with deception: {0}".format(times_full_deception)
    print >> out
...
@listener('result', handle_result)
@listener('result options', handle_options)
@listener('done', my_when_done)
class MyStatsParser(StatsParserBase):

    def __init__(self, *args, **kwdargs):
        super(MyStatsParser, self).__init__(*args, **kwdargs)
        self._effective_zero = 1e-8  # simulations had 1e-10
...
@listener('result', handle_result)
@listener('result options', handle_options)
@listener('done', my_when_done_comparative)
class MyComparativeStatsParser(StatsParserBase):

    def __init__(self, *args, **kwdargs):
        super(MyComparativeStatsParser, self).__init__(*args, **kwdargs)
        self._effective_zero = 1e-8  # simulations had 1e-10
